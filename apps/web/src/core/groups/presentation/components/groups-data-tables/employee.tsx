...
function EmployeeListDialog({ businessSlug, group }: Props) {
  const {
    data: { data: business },
  ...
  const employeesToShow = business.employees.filter(
    (user) => !group.employees.some((u) => u.id === user.id)
  );
...
function RemoveEmployees({ businessSlug, groupSlug }: RemoveEmployeesProps) {
  ...
  const handleClick = () => {
    removeEmployees.mutate({
      businessSlug,
      groupSlug,
      data: { employeeIds: selectedEmployees.map((e) => e.id) },
    });
  };
...
export function EmployeesTable({ group, businessSlug }: Props) {
